...
def extraer_datos_mensaje_mejorado(mensaje_usuario: str) -> Dict[str, Any]:
    ...
    if not datos['origen'] or not datos['destino']:
        ...
        # PRIORIDAD 6: Patrones para ubicaciones individuales "Desde X" o "Hasta Y"
        if not datos['origen']:
            patron_solo_origen = re.search(r'^(?:desde|de)\s+(.+?)(?:\s|$)', mensaje_usuario, re.IGNORECASE)
            if patron_solo_origen:
                origen_individual = patron_solo_origen.group(1).strip()
                # Filtrar palabras que no son lugares
                if not re.search(r'\b(?:mi|el|la|los|las|que|cuando|donde|como|por|para|con|sin|sobre|entre)\b', origen_individual, re.IGNORECASE):
                    datos['origen'] = origen_individual.title()
                    print(f"   ✅ Origen extraído (individual): {datos['origen']}")
        
        if not datos['destino']:
            patron_solo_destino = re.search(r'^(?:hasta|hacia|a)\s+(.+?)(?:\s|$)', mensaje_usuario, re.IGNORECASE)
            if patron_solo_destino:
                destino_individual = patron_solo_destino.group(1).strip()
                # Filtrar palabras que no son lugares
                if not re.search(r'\b(?:mi|el|la|los|las|que|cuando|donde|como|por|para|con|sin|sobre|entre)\b', destino_individual, re.IGNORECASE):
                    datos['destino'] = destino_individual.title()
                    print(f"   ✅ Destino extraído (individual): {datos['destino']}")
        
        # PRIORIDAD 7: Patrón específico para "Viña del Mar" y nombres compuestos similares
        patron_ciudades_compuestas = re.search(r'(?:de|desde|necesito\s+transporte\s+de)\s+(viña\s+del\s+mar|villa\s+del\s+mar|san\s+\w+|santa\s+\w+|los\s+\w+|las\s+\w+)\s+(?:a|al|hacia|hasta)\s+(viña\s+del\s+mar|villa\s+del\s+mar|san\s+\w+|santa\s+\w+|los\s+\w+|las\s+\w+|puerto|aeropuerto|\w+)', mensaje_usuario, re.IGNORECASE)
        if patron_ciudades_compuestas:
            # Sobreescribir resultados anteriores si detectamos ciudades compuestas
            datos['origen'] = patron_ciudades_compuestas.group(1).title()
            datos['destino'] = patron_ciudades_compuestas.group(2).title()
            print(f"   ✅ Origen extraído (ciudad compuesta SOBREESCRITO): {datos['origen']}")
            print(f"   ✅ Destino extraído (ciudad compuesta SOBREESCRITO): {datos['destino']}")
        
        # PRIORIDAD 8: Detectar si el origen extraído está incompleto (ej. "Mar" en vez de "Viña del Mar")
        if datos.get('origen') == 'Mar' and 'viña' in mensaje_usuario.lower():
            datos['origen'] = 'Viña Del Mar'
            print(f"   ✅ Origen corregido: {datos['origen']}")
        
        # PRIORIDAD 9: También detectar otros casos problemáticos
        if 'viña del mar' in mensaje_usuario.lower() and datos.get('origen', '').lower() in ['mar', 'viña']:
            datos['origen'] = 'Viña Del Mar'
            print(f"   ✅ Origen corregido (Viña del Mar): {datos['origen']}")
